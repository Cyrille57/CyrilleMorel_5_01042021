...
// 1) ////////////////////////////////////////////////////////
// Récupére les tableau des produits dans le panier:
...
async function trie (productLocalStorage) {
    console.log(productLocalStorage)
    for (var i = 0; i < productLocalStorage.length; i++) {
        let rps = productLocalStorage[i]
        console.log(rps)
        // reponse affiche les objets un a un
        //getIdProductLocalStorage(rps)
        affichessaie (rps)
    }
}
trie(productLocalStorage)

function affichessaie (rps){
    let essaiegetId =rps.idProduct
    console.log(essaiegetId)
    // Reponse : retourne les id
    assemblyId(essaiegetId)
}
...
async function assemblyId(essaiegetId) {
    //console.log(essaiegetId)
    //réponse : 5beaaa8f1c9d440000a57d95

    // Déclaration des variables:
    const url = "http://localhost:3000/api/teddies";
    const urlProduct = url + "/" + essaiegetId;
...
function ess2 (productData){

    let tableau = []
    tableau.push(productData)
    console.log (tableau)
...
/*async function getIdProductLocalStorage (rps) {
    console.log(rps)
    for (var i = 0; i < rps.length; i++) {
        console.log(rps.length)
        let idProductPanier = rps.idProduct
...
/*    let divSectionPanier = createTag('section')
    ...
    //divSectionPanier.innerHTML= "essaie"
